        ...
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="forms-sample" method="POST" action="{{ route('add-on-billiard.update', $add_ons->id) }}" enctype="multipart/form-data">
                        @method('patch')
                        @csrf
                        ...
                        <div class="modal-footer">
                            <a href="{{ route('add-on-billiard.index') }}" class="btn btn-danger p-2">
                                Kembali
                            </a>
...
<script>
    ...
    function addField() {
        ...
        $("#contactTable").find('tbody')
            .append(
                $('<tr>' +
                    '<td><select class="js-example-basic-single @error('restaurant_id') is-invalid @enderror" id="material'+rowCount+'" name="restaurant_id[]" style="width:100%"><option disabled selected>Choose Restaurant</option>@foreach ($restaurants as $restaurant)<option value="{{ $restaurant->id }}"{{ old('restaurant_id') == $restaurant->id ?'selected' : '' }}>{{ $restaurant->nama }} </option>@endforeach</select></td>' +
