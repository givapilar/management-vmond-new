...
<body>
    ...
    <div class="ticket">
        ...
        <div class="invoiceNumber">
            <div style="margin-left: 2px;">
                Datetime:
                <span style="float: right; margin-right: 15px;">{{ date('m-d-Y H:i:s') }}</span>
            </div>
        </div>
        <div class="line"></div>
        ...
        <div class="invoiceNumber">
            <div style="margin-left: 2px;">
                ...
                @elseif($orders->biliard_id)
                    ...
                    <div style="margin-left: 2px;">
                        Time Start:
                        <span style="float: right; margin-right: -50px;">{{ $orders->time_from }}</span>    
                    </div>
                    <div style="margin-left: 2px;">
                        <?php
                            $timeTo = strtotime($orders->time_to);
                            $newTimeTo = date('H:i:s', strtotime('+2 minutes', $timeTo));
                            // echo $newTimeTo;
                        ?>
                        Time End:
                        <span style="float: right; margin-right: 14px;">{{ $newTimeTo }}</span>    
                    </div>
                    {{-- <span style="float: right; margin-right: 15px;">{{ $orders->time_to }}</span>     --}}
        ...
        <table>
            ...
            @endif
                ...
                @php
                    // Harga dari paket menu atau nilai default 0 jika $firstOrderBilliard tidak tersedia
                    $menuPrice = $firstOrderBilliard->paketMenu->harga ?? 0;
                    
                    // Harga layanan berdasarkan persentase layanan dalam $other_setting
                    $layananPrice = $menuPrice * ($other_setting->layanan / 100);
                    
                    // Harga PB01 berdasarkan persentase PB01 dalam $other_setting
                    $pb01Price = ($menuPrice + $layananPrice) * ($other_setting->pb01 / 100);
                    
                    // Total Order
                    $orderTotal = $menuPrice + $layananPrice + $pb01Price;
                @endphp

                <tr>
                    <td class="quantity">&nbsp;</td>
                    <td class="description">Total</td>
                    <td class="price" style="text-align: right">Rp.{{ number_format($menuPrice, 0) }}</td>
                </tr>

                <tr>
                    <td class="quantity"></td>
                    <td class="description">Layanan</td>
                    <td class="price" style="text-align: right">Rp.{{ number_format($layananPrice, 0) }}</td>
                </tr>

                <tr style="margin-top: 20px !important;">
                    <td class="quantity"></td>
                    <td class="description">PB01</td>
                    <td class="price" style="text-align: right">Rp.{{ number_format($pb01Price, 0) }}</td>
                </tr>
