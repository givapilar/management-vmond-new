...
<body>
    ...
    <div class="ticket">
        ...
        <div class="invoiceNumber">
            <div style="margin-left: 2px;">
                Kasir:
                <span style="float: right; margin-right: 15px;">{{ $orders->nama_kasir ?? '-' }} </span>
            </div>
        </div>
        ...
        <div class="invoiceNumber">
            <div style="margin-left: 2px;">
                Time Start:
                @if($orders->biliard_id)
                    <span style="float: right; margin-right: 15px;">{{ $orders->time_from }}</span>    
                @elseif($orders->meeting_room_id)
                    <span style="float: right; margin-right: 15px;">{{ $orders->time_from }}</span> 
                @endif   
            </div>
        ...
        <div class="invoiceNumber">
            <div style="margin-left: 2px;">
                ...
                @if($orders->biliard_id)
                    <?php
                        // Tambahkan 2 menit (120 detik) ke waktu yang ada pada $orders->time_to
                        $timeTo = date('H:i:s', strtotime($orders->time_to . '+2 minutes'));
                    ?>
                    <span style="float: right; margin-right: 15px;">{{ $timeTo }}</span>    
                @elseif($orders->meeting_room_id)
                    <span style="float: right; margin-right: 15px;">{{ $orders->tableMeetingRoom->nama }}</span> 
                @endif   
            </div>
        ...
        <table>
            ...
            @if ($orders->biliard_id)
                ...
                <tr>
                    ...
                    <td class="price" style="text-align: right">Rp.{{ number_format($orders->total_price - ($orders->layanan + $orders->pb01)?? 0,0) }}</td>
                </tr>
                @else
                <tr>
            ...
            @endif
                ...
                <tr style="margin-top: 20px !important;">
                    ...
                    <td class="price" style="text-align: right">Rp.{{ number_format($orders->orderPivot->sum(function ($orderPivot)use($other_setting) {
